...
def build_weaver_mcp_server(
    ...
    @tool(
        ...
        (
            ...
            "**自动装 spec.python_deps 进 app .venv (uv add)** — 不要等 finalize, "
            "现在就把 service 需要的依赖写进 app.json components.services[].python_deps, "
            "weave_service_start 直接装 + 跑, HEIC / numpy 这种带 native binary 的库也一样. "
