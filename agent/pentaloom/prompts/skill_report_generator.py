"""report-generator skill — 内联在主 agent system prompt 里.

设计 SOP / 强制 verify / CJK 字体规约. 用 MUST/Never 强语气, 防模型偷懒跳 verify
直接说"做好了". M3 后续等 SDK skill 加载机制接上时迁到
.claude/skills/report-generator/SKILL.md, 这个文件即可删除.
"""

from __future__ import annotations

from pentaloom.tools.files import FILE_VERIFY_FULL_NAME
from pentaloom.tools.python_env import RUN_SCRIPT_FULL_NAME

# 跟 tools/system_resources.py 的常量同名 — 避免循环 import 直接写常量字符串.
_INSTALL_NOTO_SANS_SC_FULL_NAME = "mcp__pentaloom__install_noto_sans_sc"


REPORT_GENERATOR_INSTRUCTIONS: str = f"""### Skill: report-generator (PPT/报告生成)
...
4. **写脚本到 sandbox**: 用 Write 把 .py 落到当前会话 sandbox 目录里 (绝对路径,
   {RUN_SCRIPT_FULL_NAME} 接受这个). 用 python-pptx 拼 slides; 不接受 inline 代码.
5. **跑生成**: 调 {RUN_SCRIPT_FULL_NAME} 执行脚本, 拿到 .pptx 输出路径.
6. **强制 verify**: 调 {FILE_VERIFY_FULL_NAME}(path=<.pptx 路径>, autofix=True).
...
- **清单为空** → 调 {_INSTALL_NOTO_SANS_SC_FULL_NAME} 装 Noto Sans SC, 用户授权后再继续.
  不要让用户手动 brew. 装完后 verify autofix 会自动注 Noto Sans SC.
